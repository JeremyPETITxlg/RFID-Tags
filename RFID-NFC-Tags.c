    ...
    if (g_mode == MODE_AUTO) {
        ...
        while (!g_stopRequested) {
            ...
            if (ok) {
                ...
                g_currentdonnee++; // Passage à la valeur de liste suivante
                save_state(g_currentdonnee);
                PostMessage(g_hWndMain, WM_APP_donnee, (WPARAM)g_currentdonnee, 0); // Ordre rafraichissement texte compteur
            } else {
                post_log("ECHEC d'écriture pour la donnée %d. Reessayez (retirez et repositionnez le tag).", g_currentdonnee);
            }
            wait_for_card_removal_i(&g_stopRequested); // Sécurité anti-rebond (oblige un retrait)
        }
    } else if (g_mode == MODE_MANUAL) {
        // Mode Manuel : La même chaîne entrée manuellement dans l'interface sera flashée en masse
        post_log("Mode manuel actif : chaque tag présent recevra \"%s\"", g_manualValueSnapshot);
        while (!g_stopRequested) {
        ...
        // Mode Lecture : Interrogateur continu. Ne fait qu'afficher dans la console
        ...
        while (!g_stopRequested) {
            ...
            else post_log("Aucun enregistrement NDEF URI valide trouvé sur ce tag.");
        ...
        case WM_COMMAND: {
            ...
            if (id == IDC_BROWSE) {
                ...
                if (!g_isRunning) {
                    ...
                    if (!g_readerReady) {
                        MessageBox(hwnd, "Aucun lecteur détecté. Branchez le ACR122U.",
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    CreateCtrl("BUTTON", "Mode", WS_CHILD | WS_VISIBLE | BS_GROUPBOX, 10, 80, 630, 90, g_hWndMain, 0);
    ...
    SendMessage(g_hRadioAuto, BM_SETCHECK, BST_CHECKED, 0); // Paramètre Auto par défaut
